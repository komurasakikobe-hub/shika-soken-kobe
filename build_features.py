...
def ward(addr):
    m = re.search(r'神戸市\s*([^\s0-9０-９区]{1,3}区)', addr or "")
...
def main():
    ...
    ward_chips = ('<button type="button" class="ff-chip ff-ward on" data-ward="">すべて</button>'
                  + "".join(f'<button type="button" class="ff-chip ff-ward" data-ward="{esc(w)}">{esc(w.replace("神戸市", ""))}</button>'
...
<header class="odr-brandbar">
  <a class="odr-sig" href="../../index.html">
    ...
    <span class="odr-sig-name">神戸歯科総研<small>Kobe Dental Research Institute</small></span>
  </a>
  <nav>
